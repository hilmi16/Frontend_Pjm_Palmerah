...
import 'package:pjm_palmerah/screen/registrasiCheckout.dart';
...
class LoginCheckout extends StatefulWidget {
  @override
  _LoginCheckoutState createState() => _LoginCheckoutState();
}

class _LoginCheckoutState extends State<LoginCheckout> {
  String namaLengkap, id, email, password, level;
  final _key = new GlobalKey<FormState>();
  RegExp regx = RegExp(r"^[a-z0-9_]*$", caseSensitive: false);
  bool _secureText = true;
  KeranjangBelanja call = KeranjangBelanja();
        ...
        showDialog(
            ...
            builder: (context) {
              return AlertDialog(
                ...
                content: Text(
                    "Silahkan Login Menggunakan Akun User Untuk Melakukan Checkout"),
                actions: <Widget>[
                  TextButton(
                      onPressed: () {
                        Navigator.pop(context);
                ...
                InkWell(
                  onTap: () {
                    Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (context) =>
                                RegistrasiCheckout(email, fcmToken)));
                  },
                  child: Container(
                    padding: EdgeInsets.only(
                      left: displayWidth(context) * 0.04,
